...
type Cell = (i64, i64, i64, i64); // x, y, z, w
    ...
    for x in -1..2 {
        for y in -1..2 {
            for z in -1..2 {
                for w in -1..2 {
                    if (x != 0 || y != 0 || z != 0 || w != 0)
                        && cube.contains(&(c.0 + x, c.1 + y, c.2 + z, c.3 + w))
                    {
                        res += 1;
                    }
                }
...
fn dimensions(cube: &HashSet<Cell>) -> (Range<i64>, Range<i64>, Range<i64>, Range<i64>) {
    let mut x = (0, 0);
    let mut y = (0, 0);
    let mut z = (0, 0);
    let mut w = (0, 0);
